...
export default class GooglePlacesAutocomplete extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if(typeof(nextProps.text) !== "undefined" && this.state.text !== nextProps.text) {
      ...
      //custom
      if(nextProps.blurProp === true){
          this.refs.textInput.blur()
          this.refs.textInput.clear()
          this.setState({
              listViewDisplayed: false
          });
          this.props.resetBlurProp(false);
      }
  }
  ...
  _request = (text) => {
    ...
    if (text.length >= this.props.minLength) {
      ...
      request.open('GET', 'https://maps.googleapis.com/maps/api/place/autocomplete/json?&location=' + this.props.giveLocation['latitude'] + ',' + this.props.giveLocation['longitude'] + '&radius=1000&input=' + encodeURIComponent(text) + '&' + Qs.stringify(this.props.query));
      if (this.props.query.origin !== null) {
         request.setRequestHeader('Referer', this.props.query.origin)
      }
